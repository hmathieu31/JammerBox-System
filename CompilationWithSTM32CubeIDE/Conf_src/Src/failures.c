...
void output_CRK_RUN_OUT(void) {
	...
	if (failure_active == true) { // if failure is active
		if ((teeth_counter_CRK_RUN_OUT * revolution_CRK)
				>= (failure_period_CRK_RUN_OUT
						+ angle_to_start_failure_CRK_RUN_OUT)) { //once we have reached the failure end
		...
		if ((angle_to_start_failure_CRK_RUN_OUT
				< (teeth_counter_CRK_RUN_OUT * revolution_CRK))
				&& ((teeth_counter_CRK_RUN_OUT * revolution_CRK)
						< (angle_to_start_failure_CRK_RUN_OUT
								+ failure_period_CRK_RUN_OUT))) { // if we are past the the angle of fairue start and still in the failure period
			failure_active = true;
		} else {
		}
		output_CRK_no_failure();
	...
	if (active_edges_CAM_PER == 'b') {
		if (cam_id == 0) { // For CAM1
			...
			TIM_SetAutoreload(TIM3,
					(filter_time_CAM[cam_id] / 2.0 - 5)
							/ microsecond_per_timer_tick_CAM_PER__CRK_TOOTH_PER);
		} else { // For CAM2
...
void CAM_delay(int cam_id) {
	if (tim5_Counting) {
		...
		if (((double) Tim5_GetCounter() / former_teeth_time) * revolution_CRK
				>= (revolution_CRK / 2.0)) {
			if (cam_id == 0) {
	...
	if (timer_active_CAM_delay[cam_id] == true
			&& number_processing_edges_CAM_delay[cam_id] == 0) {
		...
		if (((double) SysTick->VAL / former_teeth_time) * revolution_CRK
				>= (delay_angle_CAM_delay * delay_factor_CAM_delay)) {
			...
			if (interrupt_check_CAM_delay[cam_id] == false) {
				...
				if (active_CAM_edges[cam_id] == 'r'
						|| active_CAM_edges[cam_id] == 'f') {
					SysTick->CTRL |= SysTick_CTRL_ENABLE;
				}
			}
		}
		...
		for (i = 0; i < edges_beetween_shift_CAM_delay; i++) {
			...
			if (shift_counter_CAM_delay[cam_id][i] != 0) {
				if (angle_to_edge_CAM_delay[cam_id][i]
						+ ((double) (shift_counter_CAM_delay[cam_id][i] - 1)
								+ ((double) (Tim5_GetCounter()
										+ timer_overflow_CRK
												* (unsigned long) (TIM2->ARR)))
										/ former_teeth_time) * revolution_CRK
						>= (delay_angle_CAM_delay * delay_factor_CAM_delay)) //Aurait été plus propre avec un getAutoreload
						{
			...
			for (i = 0; i < edges_beetween_shift_CAM_delay; i++) {
				if (shift_counter_CAM_delay[cam_id][i] == 0) {
					...
					angle_to_edge_CAM_delay[cam_id][i] =
							((double) (SysTick->VAL) / former_teeth_time)
									* revolution_CRK;
					SysTick->CTRL &= ~SysTick_CTRL_ENABLE;
					SysTick->VAL = (2 ^ 24) - 1;
					timer_active_CAM_delay[cam_id] = false;
...
void output_CRK_TOOTH_OFF(void) {
	...
	number_teeth_between_tooth_off = number_teeth_between_gaps
			/ number_tooth_off;
	...
	if (failure_active == true) {
		...
		if (((teeth_counter_CRK_TOOTH_OFF % number_teeth_between_tooth_off)
				== ((number_teeth_between_tooth_off / 2) + 1))
				&& (CRK_signal == true)) { //if we passed a tooth and this tooth is separated by the number_teeth_between_tooth_off
			if ((counter_gap_left_CRK_TOOTH_OFF == 0)
					&& (counter_tooth_off
							< ((number_tooth_off / number_gap)
									+ (number_tooth_off % number_gap)))) {
				failure_active = true;
...
void output_CRK_GAP_NOT_DET(void) {
	...
	if (CRK_synch == false) {
		...
		if ((teeth_counter_CRK_GAP_NOT_DET == number_teeth_between_gaps)
				&& failure_active == false) {
			failure_active = true;
...
void output_SEG_ADP_ER_LIM(void) {
	...
	if (teeth_counter_SEG_ADP_ER_LIM
			== (((720 / (number_segments_CRK * revolution_CRK))
					+ (first_seg_angle / revolution_CRK)
					+ (int) (tdc_to_gap / revolution_CRK) + 1))) { // if we are one tooth befor the end of the first segment
		double delayTimer = crk_delay_SEG_ADP_ER_LIM
				* (T_TOOTH_RAW / revolution_CRK);
