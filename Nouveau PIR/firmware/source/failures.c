			...
			switch (active_CAM_edges[cam_id])
			{
				case ('b'):
					{
						...
						{
							SysTick->CTRL |= SysTick_CTRL_ENABLE;		// Enable Timer8 till the first falling CRK_edge
				...
				case ('f'):
					{	
						...
						{
                            if(cam_id == 0){
                                HAL_GPIO_TogglePin(GPIOA, 5); //Toggle A5
                            }else if(cam_id == 1){
                               HAL_GPIO_TogglePin(GPIOA, 6); //Toggle A6
                            }
						}
